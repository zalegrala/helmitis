...
| 2b | Jsonnet authoring layer | `helm.value`/`required`/`raw`/`blockValue`, `chart.libsonnet` assembler (multi-object), generators (deployment, service, statefulset, pdb, configmap, **vpa, servicemonitor**), `workload.libsonnet`/`mounts.libsonnet`, `examples/web`, jsonnet→installable-chart gate. Config-mount (§8) done (#9). **CRD generators + kubeconform CRD validation done** (#10, via datreeio CRDs-catalog). | ✅ Done | this file |
| 3 | Tempo-flavored demo | `examples/tempo` — a capability demonstration (NOT parity with tempo-distributed): 4 microservices components (Deployment + StatefulSet), one structured `tempo.yaml` mounted across all via the config-mount, release-wide ServiceAccount; installable, gated in CI. Production descriptors would live in the Tempo repo. | ✅ Demo done (#2) | `examples/tempo/main.jsonnet` |
