...
| 2b | Jsonnet authoring layer | `helm.value`/`required`/`raw`/`blockValue` helper, `chart.libsonnet` assembler (incl. multi-object generators), generators (deployment, service, statefulset, pdb, **configmap**), `workload.libsonnet`/`mounts.libsonnet`, `examples/web`, jsonnet→installable-chart acceptance test. **Config-mount primitive (§8) done** (#9). CRD generators (vpa, servicemonitor) tracked as follow-up (#10). | 🚧 Core+config done | this file |
